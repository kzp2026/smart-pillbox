...
def _render_overview(
    ...
    runs = _cached_runs(history, 8, active) if active else []
    scoped_snapshot = _workspace_snapshot(repository, active) if active else snapshot
    ...
    st_module.markdown(panel_open_html("知识库概览", "独立 V2 私有知识库") + metric_grid_html([
        ...
        ("当前产品评论", scoped_snapshot.comment_count, "条评论", "cyan"),
        ("当前产品需求", scoped_snapshot.requirement_count, "条有效证据", "violet"),
        ("最近更新", updated, "数据库时间", "amber"),
    ]) + "</section>", unsafe_allow_html=True)

    if products:
        product_names = [item.name for item in products]
        selector_col, button_col = st_module.columns([3, 1])
        with selector_col:
            selected_product = st_module.selectbox(
                "工作产品",
                product_names,
                index=product_names.index(active) if active in product_names else 0,
                key="v2_overview_product",
            )
        with button_col:
            st_module.write("")
            if st_module.button("设为当前产品", type="primary", use_container_width=True):
                _set_active_product(st_module, selected_product)
                st_module.rerun()
    else:
        st_module.info("暂无产品。请先导入评论资产或创建设计任务。")
    ...
    with right:
        st_module.markdown(panel_open_html("下一步动作", "常用工作入口") + "</section>", unsafe_allow_html=True)
        action_left, action_right = st_module.columns(2)
        if action_left.button("打开导入评论", use_container_width=True):
            _navigate_to(st_module, "导入评论资产")
        if action_right.button("打开需求生成", type="primary", use_container_width=True):
            _navigate_to(st_module, "需求生成")
        if action_left.button("打开数据分析", use_container_width=True):
            _navigate_to(st_module, "需求-功能-结构图谱")
        if action_right.button("打开效果图", use_container_width=True):
            _navigate_to(st_module, "AI 效果图")

    st_module.markdown(panel_open_html("结果预览", f"{scoped_snapshot.artifact_count} 个归档文件") + "</section>", unsafe_allow_html=True)
    if not runs:
...
def _render_import(
    ...
    optional_columns = ["（不导入）"] + candidates
    meta_first, meta_second, meta_third = st_module.columns(3)
    rating_column = meta_first.selectbox("评分列（可选）", optional_columns)
    date_column = meta_second.selectbox("评论日期列（可选）", optional_columns)
    variant_column = meta_third.selectbox("产品版本列（可选）", optional_columns)
    meta_fourth, meta_fifth = st_module.columns(2)
    channel_column = meta_fourth.selectbox("渠道列（可选）", optional_columns)
    segment_column = meta_fifth.selectbox("用户标签列（可选）", optional_columns)
    ...
    metadata_columns = {
        ...
        "commented_at": date_column,
        "product_variant": variant_column,
        "source_channel": channel_column,
        "user_segment": segment_column,
    }
    comments = []
    comment_metadata = []
    for _, row in dataframe.iterrows():
        comment = cell_text(row[comment_column])
        if not comment:
            continue
        comments.append(comment)
        comment_metadata.append(
            {
                key: cell_text(row[column]) if column != "（不导入）" else ""
...
def _render_history(
    ...
    if runs:
        filter_left, filter_middle, filter_right = st_module.columns(3)
        status_filter = filter_left.selectbox(
            "运行状态", ["全部"] + sorted({run.status.value for run in runs})
        )
        model_filter = filter_middle.selectbox(
            "生成模型", ["全部"] + sorted({run.model for run in runs if run.model})
        )
        images_only = filter_right.checkbox("仅看有图片")
        image_run_ids = (
            _cached_view(
                repository,
                "image-run-ids",
        ...
        runs = [
            run
            for run in runs
            if (status_filter == "全部" or run.status.value == status_filter)
            and (model_filter == "全部" or run.model == model_filter)
            and (not images_only or run.id in image_run_ids)
        ]
    if not active:
        ...
        detail = _cached_run_detail(history, selected_id, False)
        ...
        with st_module.expander("结果决策", expanded=True):
            ...
            current_review = history.get_review(selected_id)
            with st_module.form(f"v2_review_{selected_id}"):
                decision = st_module.selectbox(
                    "结果决策",
                    list(decision_labels),
                    index=list(decision_labels).index(current_review.decision)
                    if current_review.decision in decision_labels
                    else 0,
                    format_func=decision_labels.get,
                )
                rating = st_module.slider("结果评分", 0, 5, int(current_review.rating))
                notes = st_module.text_area("评审备注", current_review.notes, height=90)
                is_final = st_module.checkbox("标记为最终版本", value=current_review.is_final)
                if st_module.form_submit_button("保存结果决策", type="primary"):
                    history.save_review(selected_id, decision, rating, notes, is_final)
                    st_module.success("评审结论已保存。")

        loaded_detail_id = str(st_module.session_state.get("v2_loaded_history_run_id") or "")
        if loaded_detail_id != selected_id:
            if st_module.button("加载文件预览与批量下载", icon=":material/folder_open:"):
                st_module.session_state["v2_loaded_history_run_id"] = selected_id
                st_module.rerun()
        else:
        ...
        with st_module.expander("对比两个版本", expanded=False):
            compare_ids = st_module.multiselect(
                "选择两个版本",
                [run.id for run in runs],
                max_selections=2,
                format_func=lambda run_id: (
                    f"{options[run_id].updated_at[:19].replace('T', ' ')} · {options[run_id].model}"
                ),
            )
            if len(compare_ids) == 2:
                compare_columns = st_module.columns(2)
                for column, run_id in zip(compare_columns, compare_ids):
                    compared = _cached_run_detail(history, run_id, False)
                    with column:
        ...
        with st_module.expander("需求与评论证据", expanded=False):
            evidence = repository.list_product_evidence(active, limit=20)
            if evidence["requirements"]:
                st_module.markdown("**需求证据**")
                st_module.dataframe(evidence["requirements"], hide_index=True, use_container_width=True)
            if evidence["comments"]:
                st_module.markdown("**原始评论**")
                st_module.dataframe(evidence["comments"], hide_index=True, use_container_width=True)
            if not evidence["requirements"] and not evidence["comments"]:
                st_module.caption("当前产品暂无可追溯证据。")

    st_module.divider()
    st_module.markdown("#### 恢复结果归档")
    restore_products = [item.name for item in _cached_products(repository)]
    if restore_products:
        restore_product = st_module.selectbox(
            "恢复到产品",
            restore_products,
            index=restore_products.index(active) if active in restore_products else 0,
        )
    else:
        restore_product = st_module.text_input("恢复到产品", placeholder="填写归档所属产品")
    ...
    if archive_upload:
        ...
        if st_module.button(
            ...
            disabled=not str(restore_product).strip(),
        ):
            digest = hashlib.sha256(archive_bytes).hexdigest()
            run = repository.create_pipeline_run(
                CreateRunCommand(str(restore_product).strip(), "安全 ZIP 归档恢复", "archive", "restored", 0),
                idempotency_key=f"restore:{restore_product}:{digest}",
            )
            _invalidate_view_cache(repository)
...
def _render_settings(
    ...
    st_module.divider()
    st_module.markdown("#### 管理员工具 · 原站数据复制")
    ...
    try:
        ...
        st.error(
            public_error_message(
                "私有服务初始化失败",
                exc,
                guidance="请检查连接地址、数据库密码或 Supabase 连接池凭据。",
            )
        )
        st.caption("未自动回退到公共数据库或公共存储。")
        retry_col, manage_col = st.columns(2)
        if retry_col.button("重新连接私有服务", type="primary", use_container_width=True):
            _invalidate_view_cache()
            st.rerun()
        manage_col.link_button(
            "打开 Streamlit 应用管理",
            "https://share.streamlit.io/",
            use_container_width=True,
        )
        with st.expander("连接修复检查项", expanded=True):
            st.markdown(
                "1. 在应用 Secrets 中重新粘贴完整 `V2_DATABASE_URL`。\n"
                "2. 密码含 `@`、`:`、`/`、`#` 等字符时，使用 Supabase 复制出的完整 URI。\n"
                "3. Streamlit Cloud 优先使用 Supabase Transaction pooler（端口 6543）。\n"
                "4. 保存后等待应用重启，再点击上方重新连接。"
            )
        return
