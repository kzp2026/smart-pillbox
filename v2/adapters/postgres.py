...
class KnowledgeRepository:
    ...
    def product_workspace_snapshot(self, product_name: str) -> WorkspaceSnapshot:
        """Return navigation counters scoped to one product and its runs."""
        ...
        with self.connect() as connection:
            row = connection.execute(
                self._sql(
                    "SELECT "
                    "(SELECT COUNT(*) FROM products WHERE owner_id = ? AND name = ?) AS product_count, "
                    "(SELECT COUNT(*) FROM comments c JOIN products p ON p.id = c.product_id "
                    " WHERE c.owner_id = ? AND p.owner_id = ? AND p.name = ?) AS comment_count, "
                    ...
                    "(SELECT COUNT(*) FROM artifacts a JOIN pipeline_runs pr ON pr.id = a.pipeline_run_id "
                    " WHERE a.owner_id = ? AND pr.owner_id = ? AND pr.target_product = ? AND a.kind = ?) AS image_count"
                ),
                (
                    self.owner_id,
                    product,
    ...
    def list_product_evidence(self, product_name: str, limit: int = 20) -> dict[str, list[dict]]:
        product = clean_text(product_name)
        ...
        with self.connect() as connection:
            requirements = [
                ...
                for row in connection.execute(
                    self._sql(
                        "SELECT r.title, r.description, r.keywords, r.evidence_text, r.score, r.created_at "
                        "FROM requirements r JOIN products p ON p.id = r.product_id AND p.owner_id = r.owner_id "
                        "WHERE r.owner_id = ? AND p.name = ? ORDER BY r.score DESC, r.created_at DESC LIMIT ?"
                    ),
                    (self.owner_id, product, safe_limit),
                ).fetchall()
            ]
            comments = [
                {
                    "comment": str(row["comment_original"]),
                    "created_at": str(row["created_at"]),
                    "rating": row["rating"],
                    "commented_at": str(row["commented_at"] or ""),
                    "product_variant": str(row["product_variant"] or ""),
                    "source_channel": str(row["source_channel"] or ""),
                    "user_segment": str(row["user_segment"] or ""),
                }
                for row in connection.execute(
                    self._sql(
                        "SELECT c.comment_original, c.created_at, c.rating, c.commented_at, "
                        "c.product_variant, c.source_channel, c.user_segment FROM comments c "
                        "JOIN products p ON p.id = c.product_id AND p.owner_id = c.owner_id "
                        "WHERE c.owner_id = ? AND p.name = ? ORDER BY c.created_at DESC LIMIT ?"
                    ),
                    (self.owner_id, product, safe_limit),
                ).fetchall()
            ]
        return {"requirements": requirements, "comments": comments}

    def list_run_ids_with_images(self, product_name: str) -> set[str]:
        product = clean_text(product_name)
        with self.connect() as connection:
            rows = connection.execute(
                self._sql(
                    "SELECT DISTINCT pr.id FROM pipeline_runs pr "
                    "JOIN artifacts a ON a.pipeline_run_id = pr.id AND a.owner_id = pr.owner_id "
                    "WHERE pr.owner_id = ? AND pr.target_product = ? AND a.kind = ?"
                ),
                (self.owner_id, product, ArtifactKind.IMAGE.value),
            ).fetchall()
    ...
    def save_run_review(
        ...
        allowed = {"undecided", "accepted", "revision", "rejected", "final"}
        clean_decision = clean_text(decision).lower()
        if clean_decision not in allowed:
            raise ValueError("未知的结果决策。")
        safe_rating = int(rating)
        if not 0 <= safe_rating <= 5:
            raise ValueError("结果评分必须在 0 到 5 之间。")
        now = utc_now()
        with self.connect() as connection:
            owned_run = connection.execute(
                self._sql("SELECT id FROM pipeline_runs WHERE id = ? AND owner_id = ?"),
                (pipeline_run_id, self.owner_id),
            ).fetchone()
            if not owned_run:
            ...
            connection.execute(
                self._sql(
                    ...
                    "(owner_id, pipeline_run_id, decision, rating, notes, is_final, updated_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?) "
                    "ON CONFLICT(owner_id, pipeline_run_id) DO UPDATE SET "
                    "decision = excluded.decision, rating = excluded.rating, notes = excluded.notes, "
    ...
    def get_run_review(self, pipeline_run_id: str) -> dict | None:
        with self.connect() as connection:
            row = connection.execute(
                self._sql(
                    "SELECT decision, rating, notes, is_final, updated_at FROM run_reviews "
                    "WHERE owner_id = ? AND pipeline_run_id = ?"
                ),
                (self.owner_id, pipeline_run_id),
            ).fetchone()
        return dict(row) if row else None
    ...
    def _sqlite_schema() -> list[str]:
        return [
            ...
            """CREATE TABLE IF NOT EXISTS comments (
                ...
                fingerprint TEXT NOT NULL, created_at TEXT NOT NULL, rating REAL,
                commented_at TEXT NOT NULL DEFAULT '', product_variant TEXT NOT NULL DEFAULT '',
                source_channel TEXT NOT NULL DEFAULT '', user_segment TEXT NOT NULL DEFAULT '',
            ...
            """CREATE TABLE IF NOT EXISTS run_reviews (
                owner_id TEXT NOT NULL, pipeline_run_id TEXT NOT NULL, decision TEXT NOT NULL DEFAULT 'undecided',
                rating INTEGER NOT NULL DEFAULT 0 CHECK(rating BETWEEN 0 AND 5), notes TEXT NOT NULL DEFAULT '',
                is_final INTEGER NOT NULL DEFAULT 0, updated_at TEXT NOT NULL,
