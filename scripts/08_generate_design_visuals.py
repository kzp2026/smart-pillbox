...
def maybe_enhance_image_prompts(
    ...
    prompt = f"""你是工业设计师和产品可视化提示词专家。请基于以下真实分析数据，为{product_name}生成四类写实产品渲染提示词。
...
5. 展板图要包含主效果图、三视图、爆炸结构、场景图和需求要点，但不要生成难以辨认的大段文字。
6. 每条提示词都必须强调 photorealistic、industrial design visualization、no logo、no watermark。
7. 严格按下列 Markdown 格式输出，只能包含四个章节，每个章节恰好一行以 Prompt: 开头的完整提示词：
...
def main() -> None:
    ...
    # 生成图像提示词
    base_prompts = build_image_prompts(product_name, req_df)
    prompts_text, prompt_method = maybe_enhance_image_prompts(
        base_prompts, product_name, req_df, topic_df
    )
