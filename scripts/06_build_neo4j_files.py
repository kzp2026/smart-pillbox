...
def main() -> None:
    """第六阶段：生成 Neo4j 可导入节点表、关系表和 Cypher 文件。"""
    parser = argparse.ArgumentParser(description="第六阶段：生成 Neo4j 导入文件")
    ...
    product_id = "PRODUCT_智能药盒"
    add_node(nodes, product_id, "Product", "智能药盒", "研究对象", "基于用户评论数据的智能药盒产品设计研究对象", 1.0, "论文实验")
    ...
    for _, row in req_func_df.iterrows():
        relationships.append({
            "source_id": row.get("req_id", ""),
            "target_id": row.get("func_id", ""),
            "type": "SATISFIED_BY",
            "weight": row.get("映射强度", 1),
            "reason": row.get("映射理由", ""),
        })

    for _, row in func_struct_df.iterrows():
        relationships.append({
            "source_id": row.get("func_id", ""),
            "target_id": row.get("structure_id", ""),
            "type": "REALIZED_BY",
            "weight": 1,
            "reason": row.get("映射理由", ""),
        })
    ...
    for _, row in topic_df.iterrows():
        ...
        add_node(
            ...
            topic_id,
            "Topic",
            str(row.get("主题名称", topic_id)),
            "用户评论主题",
            str(row.get("主题关键词", "")),
            float(row.get("评论数", 1) or 1),
            str(row.get("algorithm", "")),
        )

    for _, row in topic_req_df.iterrows():
        relationships.append({
            "source_id": f"TOPIC_{row.get('topic_id', '')}",
            "target_id": row.get("req_id", ""),
        ...
        for _, req in req_df.iterrows():
            if keyword in str(req.get("来源关键词", "")):
                relationships.append({
                    ...
                    "target_id": req.get("req_id", ""),
                    "type": "MENTIONS_KEYWORD",
                    "weight": row.get("TF-IDF权重", 1),
                    "reason": "关键词支撑该用户需求",
                })

    # 清理空关系，防止 Neo4j 导入时匹配不到节点。
    node_ids = set(nodes)
    relationships = [
        rel for rel in relationships
        if str(rel.get("source_id", "")) in node_ids and str(rel.get("target_id", "")) in node_ids
    ]
