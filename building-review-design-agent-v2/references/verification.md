...
| 切页性能 | 页面切换 < 3 秒；切页重跑不重复初始化仓库；非概览页不额外读取全局快照；全局/产品计数各自单连接单查询；历史、图谱、设计方案与效果图首屏不读取归档/大文件；30 秒内快照、产品、运行与详情不重复加载；写入和退出后缓存失效；2 项运行时 WebP data URI 合计 < 900 KB 且只编码一次 |
...
| 后台图像与图谱 | `ImageJobRegistry` 启动必须在 0.2 秒内返回且同一运行不重复启动；图像调用不占用 Streamlit 页面线程；文字生成持久化需求-功能-结构图谱，旧运行也必须显示兼容图谱；进程重启后的运行中图像任务只提示重新生成 |
