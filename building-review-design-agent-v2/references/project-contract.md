...
- 在线文字生成提示不得直接序列化完整评论库、评论正文、候选来源编号集合或旧方案全文。输入上限为 12,000 个字符，只保留被检索评论的稳定编号、批次和排序得分，以及需求、功能、结构和审核状态。完整原文仅在私有评论库中通过运行编号追溯。
- 设计方案、工业设计 Prompt、逐图 Prompt 与历史记录中的对应预览和下载不得显示原始评论正文。新运行的提示词不写入评论正文。旧运行仅在展示和提示词下载时替换历史嵌入的评论片段，不覆盖原记录。
