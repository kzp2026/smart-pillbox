...
上传、下载、链接、表单提交、主次按钮、密码可见切换、代码复制和下拉选项必须使用同一深色控制台色板，包含高对比文字、hover、disabled 与 `:focus-visible` 状态；不得出现与页面冲突的白底按钮。

付费图片默认值为 1 张，0 张可只生成文字方案；确认区必须显示最大付费调用数量并要求显式确认。逐图完成后立即持久化并更新进度，单图失败不得丢失已成功图片。
