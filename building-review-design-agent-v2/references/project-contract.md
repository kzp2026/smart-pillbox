...
- 侧栏提供“配置百炼效果图 Key”直达入口；设置页只展示 `V2_IMAGE_API_KEY` 占位模板并跳转 Streamlit 应用管理，不接收、不保存、不回显页面输入的 Key。DeepSeek 已有配置不要求重复填写。
...
- 运行时 WebP 的 base64 编码结果按进程缓存。AI 效果图页面先读取运行与资产元数据并完成切页，只有用户点击“加载效果图预览”后才读取图片字节；数据库私有 blob 使用单连接批量读取，禁止逐图片新建连接。
- 当前产品保存在会话上下文中。输入或创建新产品时清除上一产品的当前运行和图片预览状态；设计方案、Prompt、效果图与历史只查询当前产品。历史记录未删除，只有在用户主动选择其他产品时才显示该产品记录。
...
- 工作台产品、评论、需求、生成与图片进度由 `workspace_snapshot()` 在一个数据库连接和一条聚合查询中返回，禁止导航栏按表逐次查询。
...
- `v2/assets/*.png` 保留为视觉源文件；运行时使用对应的 `studio-background.webp`、`ai-brand-mark.webp`、`assistant-mascot.webp`。三项 data URI 总载荷必须小于 900 KB，避免每次 Streamlit 重跑重新发送多 MB 视觉资源。
...
上传、下载、链接、表单提交、主次按钮、密码可见切换、代码复制和下拉选项必须使用同一深色控制台色板，包含高对比文字、hover、disabled 与 `:focus-visible` 状态；不得出现与页面冲突的白底按钮。移动端隐藏固定助手，避免遮挡主操作。
